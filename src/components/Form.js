    ...
    return (
        <>
            <div className={`my-5 container text-${props.theme==='light'?'dark':'light'}`}>
                ...
                <button disabled={props.text.length === 0} className="btn btn-info mx-3 my-2" onClick={ handleLowClick }>Convert to Lowercase</button>
                <button disabled={props.text.length === 0} className="btn btn-success my-2" onClick={ handleRemoveSpaceClick }>Remove Extra Spaces</button>
                <button disabled={props.text.length === 0} className="btn btn-danger mx-3 my-2" onClick={ handleClearClick }>Clear Text</button>
